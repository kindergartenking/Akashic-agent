...
export type ChatFrame =
  ...
  | { type: "agent.delegation.started"; session_id: string; turn_id: string; agent_id: string; task: string }
  | { type: "agent.delegation.completed"; session_id: string; turn_id: string; agent_id: string; status: string }
...
export interface WebChatFrameContext {
  ...
  loadMessages: (sessionId: string, options?: { preserveLive?: boolean }) => Promise<void>;
}
...
export function parseChatFrame(value: unknown): ChatFrame {
  ...
  switch (frame.type) {
    ...
    case "agent.delegation.started":
      requireStrings(frame, ["session_id", "turn_id", "agent_id", "task"]);
      break;
    case "agent.delegation.completed":
      requireStrings(frame, ["session_id", "turn_id", "agent_id", "status"]);
      break;
...
export function applyChatFrame(frame: ChatFrame, context: WebChatFrameContext): void {
  ...
  // Delegation events are part of the wire contract even though this MVP does
  // not render a dedicated child-agent card yet. Tool activity is rendered by
  // the react.tool.* events below.
  if (frame.type === "agent.delegation.started" || frame.type === "agent.delegation.completed") return;
  ...
  // Keep the callback for compatibility with the full transport, but tell the
  // MVP controller not to replace a live in-memory stream with empty history.
  void context.loadMessages(frame.session_id, { preserveLive: true });
  void context.loadSessions();
}
