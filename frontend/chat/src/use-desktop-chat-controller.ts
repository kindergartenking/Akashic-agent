...
export function useDesktopChatController() {
  ...
  const loadMessages = useCallback(async (sessionId: string, options: { preserveLive?: boolean } = {}) => {
    ...
    try {
      ...
      if (options.preserveLive && page.items.length === 0 && messagesRef.current.length > 0) {
        return;
      }
      streamStore.clear();
  ...
  const sendMessage = useCallback(async (text: string, files: ComposerFile[]) => {
    ...
    try {
      ...
      const payload: Record<string, unknown> = {
        ...
        // The client creates one stable turn identity per Enter/send action;
        // the backend persists this same id with both user and assistant rows.
        turn_id: `turn-${createUuid().replaceAll("-", "")}`,
        text: cleanText,
